					...
					if (clickAmounts == 2)
					{
						if (icons[i] == "story mode is idiot")
						{
							...
							new FlxTimer().start(.5, function(tmr:FlxTimer)
							{
								...
								PlayState.SONG = important.Song.loadFromJson(PlayState.storyPlaylist[0].toLowerCase() + diffic,
...
class RunTab extends FlxGroup
{
	...
	override function update(elapsed)
	{
		...
		if (tabBar.status == 2)
		{
			...
			{
				justMousePos = FlxG.mouse.getScreenPosition();
				justTaskBarPos.set(tab.x, tab.y);
				movingTab = true;
			}
		}
		...
		if (movingTab)
		{
			tab.setPosition(Math.round(FlxG.mouse.getScreenPosition().x - justMousePos.x) + justTaskBarPos.x,
				Math.round(FlxG.mouse.getScreenPosition().y - justMousePos.y) + justTaskBarPos.y);
			for (button in [ok, cancel, help, exit, tabBar, field])
			{
				var offsetIndex:Map<Dynamic, Dynamic> = [
					ok => [177, 125],
					cancel => [258, 125],
					help => [308, 6],
					exit => [327, 6],
	...
	function triggerRunEvent(runText:String)
	{
		...
		{
			case "teevee":
				CoolUtil.browserLoad("https://youtu.be/X9hIJDzo9m0");
			case "ron": #if windows Sys.command("start RON.exe"); #end
			case "full" | "full version" | "2.5" | "3.0" | "demo 3" | "next demo":
				CoolUtil.browserLoad("https://youtu.be/pNzGTCEmf3U");
			case "2012":
				rainbowscreen.visible = false;
				FlxG.sound.play(Paths.sound('vine'));
			case "winver":
				FlxG.state.add(new Winver());
			case "cdplayer":
				FlxG.state.add(new MusicPlayer());
				FlxG.sound.music.volume = 0.01;
			case "passionatedevs":
				ClientPrefs.rtxMode = !ClientPrefs.rtxMode;
			default:
				if (runText.contains("www") || runText.contains("http") || runText.contains("com"))
					CoolUtil.browserLoad(runText);
		}
	}
}
...
class MusicPlayer extends FlxGroup
{
	...
	public function new()
	{
		...
		forward = new FlxButton(tab.x + 199, tab.y + 54, function()
		{
			if (ronmusic.playing)
			{
				if (ronmusic.time + 3000 > ronmusic.length)
					ronmusic.stop();
				else
					ronmusic.time += 3000;
			}
		...
		dropDown = new FlxUIDropDownMenu(tab.x + 47, tab.y + 135, FlxUIDropDownMenu.makeStrIdLabelArray(sys.FileSystem.readDirectory("assets/songs")),
			function(song)
			{
				ronmusic.stop();
				ronmusicvox.stop();
	...
	override function update(elapsed)
	{
		...
		timer.text = "] " + (Math.floor((timey / 100000) * 1.6666) < 10 ? "0" : "") + Math.floor((timey / 100000) * 1.6666)
			+ (Math.floor((timey / 1000) % 60) < 10 ? ":0" : ":") + Math.floor((timey / 1000) % 60);
		if (movingTab)
		{
			tab.setPosition(Math.round(FlxG.mouse.getScreenPosition().x - justMousePos.x) + justTaskBarPos.x,
				Math.round(FlxG.mouse.getScreenPosition().y - justMousePos.y) + justTaskBarPos.y);
